...
#[test]
fn cancelled_vector_index_finalization_removes_partial_state() {
    let engine = make_engine();
    let index_name = "person_embedding_idx";
    let catalog = IndexCatalog::new(engine.storage.as_ref());
    catalog
    ...
    engine
        .storage
        .persist_index_options(index_name, &options)
        .unwrap();
    engine
        .vector_indexes
        .create_index(index_name, 3, HnswConfig::default())
        .unwrap();
    let request_context = RequestContext::detached();
    request_context.cancel();

    let error = engine
        .finalize_vector_index(&request_context, index_name, &options)
        .unwrap_err();
...
#[test]
fn row_projection_observes_installed_request_cancellation() {
    let engine = make_engine();
    let row = HashMap::from([("name".into(), Value::String("Alice".into()))]);
    ...
    let error = engine
        .with_request_context(&request_context, || {
            engine.project_row(&row, &items, &HashMap::new())
        })
        .unwrap_err();
    assert!(matches!(error, EvalError::RequestCancelled(_)));

    assert_eq!(
        engine.project_row(&row, &items, &HashMap::new()).unwrap(),
        row
    );
}

#[test]
fn unwind_batch_observes_installed_request_cancellation_before_mutation() {
    let engine = make_engine();
    let query = Parser::new()
        .parse("UNWIND $rows AS row MERGE (n:Star {starId: row.starId}) SET n.name = row.name")
        .unwrap();
    let params = HashMap::from([(
        "rows".to_string(),
        Value::Array(vec![serde_json::json!({"starId": "s1", "name": "Sirius"})]),
    )]);
    let request_context = RequestContext::detached();
    request_context.cancel();
    ...
    let error = engine
        .with_request_context(&request_context, || {
            engine.execute_unwind_simple_merge_set_batch(&query, &params)
        })
        .unwrap_err();
...
fn test_execute_with_routes_optimizes_mutual_relationships() {
    ...
    let cypher =
        "MATCH (a:Person)-[:FOLLOWS]->(b:Person)-[:FOLLOWS]->(a) RETURN a.name AS a, b.name AS b";
    let query = parser.parse(cypher).unwrap();
    let pattern = detect_query_pattern(cypher);
...
fn test_execute_with_routes_optimizes_untyped_incoming_count_aggregation() {
    ...
    let cypher =
        "MATCH (p:Person)<-[r]-(f:Person) RETURN p.name AS person, count(f) AS followers LIMIT 2";
    let query = parser.parse(cypher).unwrap();
    let pattern = detect_query_pattern(cypher);
...
fn raw_count_all_relationships_uses_the_storage_executor_fast_path() {
    ...
    let result = engine
        .execute_cypher("MATCH ()-[r]->() RETURN count(r) AS count", &HashMap::new())
        .unwrap();
...
fn prof_raw_two_hop_match_count_benchmark_breakdown() {
    ...
    const QUERY: &str = "MATCH (a)-[:KNOWS]->(b)-[:KNOWS]->(c) RETURN count(c) AS count";
    ...
    for edge in &first_hop_edges {
        if engine
            .storage
            .get_node_record(&edge.start_node)
            .unwrap()
            .is_none()
            || engine
                .storage
                .get_node_record(&edge.end_node)
                .unwrap()
                .is_none()
        {
            continue;
    ...
    assert_eq!(
        cached_result.rows[0].get("count"),
...
fn test_unwind_merge_set_batch_duplicate_keys_last_row_wins_and_upserts() {
    ...
    let s2 = stored
        ...
        .unwrap();
    assert_eq!(
        s2.properties.get("name"),
        Some(&Value::String("still-two".into()))
    );
...
fn test_pipeline_route_with_then_return_aggregation_collapses_rows() {
    ...
    let cypher =
        "MATCH (a:A) WITH count(a) AS aCount MATCH (b:B) RETURN aCount, count(b) AS bCount";
    let query = parser.parse(cypher).unwrap();
    let pattern = detect_query_pattern(cypher);
    let (clauses, ok) = can_execute_as_pipeline(cypher);
...
fn test_unwind_match_merge_relationship_set_batch_idempotent_last_row_wins() {
    ...
    assert_eq!(result.stats.relationships_created, 0);
    assert_eq!(
        engine.storage.get_edges_by_type("HYPERLANE").unwrap().len(),
        2
    );
...
fn test_match_variable_length_exact_hops_and_edge_list_binding() {
    ...
    let query = parser
        .parse("MATCH (a:Node {name: 'a'})-[r:LINK*2]->(n:Node) RETURN n.name AS name, r AS rels")
        .unwrap();
    let result = engine.execute(&query, &HashMap::new()).unwrap();
...
fn raw_read_result_cache_is_invalidated_by_graph_mutation() {
    ...
    const QUERY: &str = "MATCH (a)-[:LINK]->(b)-[:LINK]->(c) RETURN count(c) AS count";
...
fn test_execute_with_routes_shortest_path_demo_mesh_performance() {
    ...
    let params = HashMap::from([
        ...
        ("endId".to_string(), Value::String(format!("s{star_count}"))),
    ]);
    ...
    for _round in 0..iters {
        ...
        for i in 0..node_count {
            nodes.push(NodeRecord {
                ...
                properties: BTreeMap::from([("idx".to_string(), Value::from(i as i64))]),
                named_embeddings: BTreeMap::new(),
                chunk_embeddings: Vec::new(),
                embed_meta: Default::default(),
                created_at_unix_ms: 0,
        ...
        let t0 = std::time::Instant::now();
        let adjacency = engine
            .bfs_adjacency_map(&rel_types, &direction, &resolver)
            .unwrap();
        ...
        let result = engine.execute(&parsed, &params).unwrap();
