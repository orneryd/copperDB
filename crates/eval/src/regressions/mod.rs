...
fn test_set_relationship_property_persists_edge_binding() {
    ...
    engine
        .execute(
            &parser
                .parse(
                    "MATCH (p1:Person {id: 1}), (p2:Person {id: 2}) CREATE (p1)-[r:FOLLOWS]->(p2)",
                )
                .unwrap(),
            &HashMap::new(),
        )
    ...
    assert_eq!(
        stored[0].properties.get("weight"),
...
fn test_relationship_temporal_index_supports_simple_where_comparisons() {
    ...
    engine
        .execute(
            &parser
                .parse(
                    "CREATE TEMPORAL INDEX follows_seen_at_idx FOR ()-[r:FOLLOWS]-() ON (r.seenAt)",
                )
                .unwrap(),
            &HashMap::new(),
        )
