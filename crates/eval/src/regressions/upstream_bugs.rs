...
fn test_return_after_set_returns_updated_value() {
    ...
    engine
        .execute(
            &parser
                .parse("CREATE (s:Step {title: 'original', content: 'old content'})")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
...
fn test_return_after_detach_delete_returns_literal() {
    ...
    engine
        .execute(
            &parser
                .parse("CREATE (s:Step {title: 'to-delete'})")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    let result = engine
        .execute(
            &parser
                .parse(
                    "MATCH (s:Step {title: 'to-delete'}) DETACH DELETE s RETURN 'done' AS result",
                )
                .unwrap(),
            &HashMap::new(),
        )
...
fn test_detach_delete_connected_node_succeeds() {
    ...
    assert_eq!(
        remaining.rows,
        vec![HashMap::from([("name".into(), Value::from("peer"))])]
    );
}
...
fn test_create_evaluates_to_string_concatenation_property() {
    ...
    assert_eq!(
        ...
        Some(&Value::String("t0".to_string()))
    );
    assert_eq!(
...
fn test_multihop_optional_match_returns_rows_for_unmatched() {
    ...
    engine
        .execute(
            &parser
                .parse("CREATE (p:Protocol {trigger: 'end session'})")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    assert_eq!(
        result.rows[0].get("title"),
        Some(&Value::String("step1".to_string()))
    );
    assert_eq!(
        result.rows[0].get("newer_title"),
        Some(&Value::String("step2".to_string()))
    );
    assert_eq!(
        result.rows[1].get("title"),
        Some(&Value::String("step2".to_string()))
    );
    assert_eq!(result.rows[1].get("newer_title"), Some(&Value::Null));
    assert_eq!(
        result.rows[2].get("title"),
        Some(&Value::String("step3".to_string()))
    );
...
fn test_relationship_function_projection_without_optional_match() {
    ...
    assert_eq!(result.rows.len(), 1);
    assert_eq!(
        result.rows[0].get("relationship_type"),
        Some(&Value::from("INHERITS"))
    );
    assert_eq!(result.rows[0].get("target_name"), Some(&Value::from("Dog")));
}
...
fn test_relationship_match_in_list_seeds_from_property_index() {
    ...
    engine
        .execute(
            &parser
                .parse(
                    "CREATE INDEX resource_uid_idx IF NOT EXISTS FOR (n:CloudResource) ON (n.uid)",
                )
                .unwrap(),
            &HashMap::new(),
        )
    ...
    assert_eq!(
        result.rows[0].get("target_uid"),
        Some(&Value::from("selected-a"))
    );
    assert_eq!(result.rows[1].get("uid"), Some(&Value::from("selected-b")));
    assert_eq!(
        result.rows[1].get("target_uid"),
        Some(&Value::from("selected-b"))
    );
    assert!(
        engine
...
fn test_typed_undirected_relationship_existence_respects_type() {
    ...
    let result = engine
        .execute(
            &parser
                .parse(
                    "MATCH (n:Item) WHERE (n)-[:CONTAINS]-() RETURN n.name AS name ORDER BY name",
                )
                .unwrap(),
            &HashMap::new(),
        )
    ...
    assert_eq!(
        ...
        Some(&Value::from("contains-end"))
    );
    assert_eq!(
        result.rows[1].get("name"),
        Some(&Value::from("contains-start"))
    );
}
...
fn test_disconnected_optional_node_match_joins_or_null_fills() {
    ...
    assert_eq!(
        matching.rows[0].get("relationship_type"),
        Some(&Value::from("INHERITS"))
    );
    assert_eq!(
        matching.rows[0].get("target_name"),
        Some(&Value::from("Dog"))
    );
    assert_eq!(
        matching.rows[0].get("repo_id"),
        Some(&Value::from("repo:1"))
    );
    ...
    assert_eq!(
        unmatched.rows[0].get("target_name"),
        Some(&Value::from("Dog"))
    );
...
fn test_disconnected_optional_relationship_match_joins_independently() {
    ...
    assert_eq!(
        result.rows[0].get("relationship_type"),
        Some(&Value::from("REPO_CONTAINS"))
    );
    assert_eq!(
        result.rows[0].get("file_path"),
        Some(&Value::from("svc.py"))
    );
}
...
fn test_optional_match_stdev_and_stdevp_aggregates() {
    ...
    assert_eq!(
        single_value.rows[0].get("population"),
        Some(&Value::from(0.0))
    );
...
fn test_unwind_merge_single_node_inline_list() {
    ...
    assert_eq!(result.rows.len(), 2);
    assert_eq!(
        result.rows[0].get("linked"),
        Some(&Value::String("aa".to_string()))
    );
    assert_eq!(
        result.rows[1].get("linked"),
        Some(&Value::String("bb".to_string()))
    );
}
...
fn test_unwind_merge_dual_no_order() {
    ...
    let mut linked: Vec<String> = result
        .rows
        .iter()
        .map(|r| r.get("linked").and_then(Value::as_str).unwrap().to_string())
        .collect();
    linked.sort();
...
fn test_match_unwind_param_merge_expands_list_values() {
    ...
    params.insert(
        "names".to_string(),
        Value::Array(vec![
            Value::String("proj-a".to_string()),
            Value::String("proj-b".to_string()),
        ]),
    );
    let result = engine
            .execute(
...
fn test_match_comma_create_where_not_edge() {
    ...
    engine
        .execute(
            &parser
                .parse("CREATE (t:Task {project:'dimension'})")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    engine
        .execute(
            &parser.parse("CREATE (s:Session {id:'sid2026'})").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    let result = engine
    ...
    let verify = engine
        .execute(
            &parser
                .parse("MATCH (t:Task)-[r:RAISED_IN]->(s:Session) RETURN count(r) AS c")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
...
fn test_match_create_node_to_matched_node() {
    ...
    engine
        .execute(
            &parser.parse("CREATE (s:Session {id:'sid'})").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    let result = engine
        .execute(
            &parser
                .parse("MATCH (s:Session {id:'sid'}) CREATE (n:Foo)-[:RAISED_IN]->(s) RETURN n")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    assert_eq!(result.stats.relationships_created, 1);
    let verify_nodes = engine
        .execute(
            &parser.parse("MATCH (n:Foo) RETURN count(n)").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    assert_eq!(verify_nodes.rows[0].get("count(n)"), Some(&Value::from(1)));
    let verify_rels = engine
        .execute(
            &parser
                .parse("MATCH (:Foo)-[r:RAISED_IN]->(:Session) RETURN count(r)")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
...
fn test_set_self_string_concat() {
    ...
    engine
        .execute(
            &parser
                .parse("CREATE (h:Heuristic {content:'hello'})")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let got = engine
        .execute(
            &parser
                .parse("MATCH (h:Heuristic) RETURN h.content")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
...
fn test_set_map_replacement_preserves_properties() {
    ...
    let result = engine
        .execute(
            &parser
                .parse("MERGE (n:Bug {uuid: $uuid}) SET n = $d RETURN properties(n) AS p")
                .unwrap(),
            &params,
        )
        .unwrap();
...
fn test_param_map_property_access_return() {
    ...
    let result = engine
        .execute(&parser.parse("RETURN $d.uuid AS uuid").unwrap(), &params)
        .unwrap();
    assert_eq!(result.rows.len(), 1);
    assert_eq!(
        result.rows[0].get("uuid"),
...
fn test_param_property_access_in_merge() {
    ...
    let result = engine
        .execute(
            &parser
                .parse("MERGE (n:Bug {uuid: $d.uuid}) SET n = $d RETURN properties(n) AS p")
                .unwrap(),
            &params,
        )
        .unwrap();
...
fn test_chained_set_label_then_map() {
    ...
    assert_eq!(result.rows.len(), 1);
    let p = result.rows[0].get("p").and_then(Value::as_object).unwrap();
    assert_eq!(p.get("uuid"), Some(&Value::String(key.to_string())));
...
fn test_case_expression_searched() {
    ...
    for (name, age) in [("Alice", 15), ("Bob", 30), ("Charlie", 70), ("Diana", 25)] {
        engine
            .execute(
                &parser
                    .parse(&format!("CREATE (p:Person {{name: '{name}', age: {age}}})"))
                    .unwrap(),
                &HashMap::new(),
            )
            .unwrap();
...
fn test_case_expression_no_else() {
    ...
    for row in &result.rows {
        ...
        if name == "Bob" {
            ...
            assert!(
                cat.and_then(Value::as_str).is_some(),
                "{name} should have category"
            );
        }
    }
}
...
fn test_case_expression_in_with() {
    ...
    engine
        .execute(
            &parser.parse("CREATE (n:Val {value: 10})").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    engine
        .execute(
            &parser.parse("CREATE (n:Val {value: 3})").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    let result = engine
...
fn test_case_expression_is_not_null() {
    ...
    engine
        .execute(
            &parser.parse("CREATE (n:Node {name: 'hasValue'})").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    engine
        .execute(&parser.parse("CREATE (n:Node)").unwrap(), &HashMap::new())
        .unwrap();
    let result = engine
            .execute(
                &parser
...
fn test_list_functions() {
    ...
    let result = engine
        .execute(
            &parser.parse("RETURN range(1, 5) AS r").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser.parse("RETURN head([1, 2, 3]) AS h").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser.parse("RETURN tail([1, 2, 3]) AS t").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser.parse("RETURN last([1, 2, 3]) AS l").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser.parse("RETURN reverse([1, 2, 3]) AS r").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    let arr = result.rows[0].get("r").and_then(Value::as_array).unwrap();
...
fn test_math_functions() {
    ...
    let result = engine
        .execute(
            &parser.parse("RETURN abs(-5) AS a").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser.parse("RETURN sign(-10) AS s").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser.parse("RETURN sqrt(16) AS s").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
...
fn test_predicate_functions() {
    ...
    let result = engine
        .execute(
            &parser.parse("RETURN none([]) AS n").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser.parse("RETURN single([1]) AS s").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser.parse("RETURN single([1, 2]) AS s").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
...
fn test_arithmetic_operators() {
    ...
    let result = engine
        .execute(
            &parser.parse("RETURN 10 / 3 AS d").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser.parse("RETURN 10 % 3 AS m").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
...
fn test_trig_functions() {
    ...
    let result = engine
        .execute(
            &parser.parse("RETURN sin(0) AS s").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser.parse("RETURN cos(0) AS c").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
...
fn test_power_functions() {
    ...
    let result = engine
        .execute(
            &parser.parse("RETURN pow(2, 3) AS p").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser.parse("RETURN exp(1) AS e").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    let e = result.rows[0].get("e").and_then(Value::as_f64).unwrap();
    assert!(
        (e - std::f64::consts::E).abs() < 0.001,
        "exp(1) should be e"
    );
}
...
fn test_null_safe_and_type_functions() {
    ...
    let result = engine
        .execute(
            &parser.parse("RETURN nullIf('a', 'b') AS n").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser.parse("RETURN nullIf('a', 'a') AS n").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser.parse("RETURN valueType(42) AS v").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    assert_eq!(
    ...
    let result = engine
        .execute(
            &parser.parse("RETURN valueType('hi') AS v").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    assert_eq!(
    ...
    let result = engine
        .execute(
            &parser.parse("RETURN char_length('hello') AS c").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
...
fn test_slice_and_indexof() {
    ...
    let result = engine
        .execute(
            &parser
                .parse("RETURN slice([1, 2, 3, 4, 5], 1, 3) AS s")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser
                .parse("RETURN indexOf([10, 20, 30], 20) AS i")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser
                .parse("RETURN indexOf([10, 20, 30], 99) AS i")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
...
fn test_reduce_function() {
    ...
    let result = engine
        .execute(
            &parser
                .parse("RETURN reduce(acc = 0, x IN [1, 2, 3] | acc + x) AS total")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser
                .parse("RETURN reduce(s = '', w IN ['a', 'b', 'c'] | s + w) AS concat")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    assert_eq!(
        result.rows[0].get("concat"),
    ...
    let result = engine
        .execute(
            &parser
                .parse("RETURN reduce(acc = 1, x IN [1, 2, 3] | acc * x) AS product")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
...
fn test_between_operator() {
    ...
    let result = engine
        .execute(
            &parser.parse("RETURN 5 BETWEEN 1 AND 10 AS b").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    assert_eq!(result.rows[0].get("b"), Some(&Value::Bool(true)));
    ...
    let result = engine
        .execute(
            &parser.parse("RETURN 15 BETWEEN 1 AND 10 AS b").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
...
fn test_vector_similarity() {
    ...
    assert!(
        (e - 1.0).abs() < 0.001,
        "identical vectors should have euclidean 1.0"
    );
}
...
fn test_vector_search_index_hot_path() {
    ...
    engine
        .execute(
            &parser
                .parse("CREATE (a:Doc {name: 'a', emb: [1.0, 0.0, 0.0]})")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    engine
        .execute(
            &parser
                .parse("CREATE (c:Doc {name: 'c', emb: [1.0, 0.0, 0.0]})")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
...
fn test_graphiti_bulk_node_save_with_vectors() {
    ...
    for row in &result.rows {
        ...
        assert!(
            emb.is_some(),
            "each node should have a name_embedding vector"
        );
    }
}
    ...
    for i in 0..20 {
        ...
        engine
            .execute(
                &parser
                    .parse(&format!(
                        "CREATE (c:Chunk {{uuid:'c-{i}', group_id:'g', emb: {v:?}}})"
                    ))
                    .unwrap(),
                &HashMap::new(),
            )
...
fn test_northwind_products_seed_two_match_create_keeps_fast_path() {
    ...
    assert!(ok);
    let wal_entries_before = engine.storage.wal_stats().entries;
    let result = engine
        .execute_with_routes(
    ...
    assert_eq!(engine.storage.wal_stats().entries, wal_entries_before + 1);
    assert!(
        engine
            .hot_path_trace_snapshot()
...
fn test_fulltext_query_relationships() {
    ...
    engine
        .execute(
            &parser.parse("CREATE (a:Entity {name: 'A'})").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    engine
        .execute(
            &parser.parse("CREATE (b:Entity {name: 'B'})").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    engine
    ...
    assert!(
        !result.rows.is_empty(),
        "should find authentication relationship"
    );
}
...
fn test_fulltext_query_relationships_honors_options_map() {
    ...
    for query in [
        ...
        engine
            .execute(&parser.parse(query).unwrap(), &HashMap::new())
            .unwrap();
    }
...
fn test_fulltext_query_relationships_temporal_filter() {
    ...
    engine
        .execute(
            &parser
                .parse("CREATE (a:Entity {name: 'A'}), (b:Entity {name: 'B'})")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    assert!(
        !result.rows.is_empty(),
        "should find at least one knows relationship passing temporal filter"
    );
    for row in &result.rows {
        let name = row.get("name").and_then(Value::as_str).unwrap_or("");
        assert!(
            name.contains("knows"),
            "only 'knows' edges should match fulltext"
        );
    }
}
...
fn test_vector_relationship_index_search() {
    ...
    assert!(
        !result.rows.is_empty(),
        "should find vector-similar relationship"
    );
}
...
fn test_vector_procedures_node_and_relationship_manual_vector_parity_e2e() {
    ...
    let query_params = HashMap::from([("v".to_string(), serde_json::json!([0.9, 0.1, 0.0, 0.0]))]);
    ...
    assert_eq!(
        node_hits.rows[0].get("u"),
        Some(&Value::String("n1".to_string()))
    );
    assert_eq!(
        node_hits.rows[1].get("u"),
        Some(&Value::String("n2".to_string()))
    );
    assert!(
        node_hits.rows[0]
            .get("score")
    ...
    assert_eq!(
        rel_hits.rows[0].get("u"),
        Some(&Value::String("z1".to_string()))
    );
    assert_eq!(
        rel_hits.rows[1].get("u"),
        Some(&Value::String("z2".to_string()))
    );
    assert!(
        rel_hits.rows[0]
            .get("score")
...
fn test_foreach_set() {
    ...
    engine
        .execute(
            &parser
                .parse("CREATE (a:Item {id: 'a'}), (b:Item {id: 'b'})")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    let result = engine
        .execute(
            &parser
                .parse(
                    "MATCH (n:Item) FOREACH (x IN [1] | SET n.marked = true) RETURN n.id, n.marked",
                )
                .unwrap(),
            &HashMap::new(),
        )
...
fn test_rel_vector_index_syntax_variants() {
    ...
    assert!(
        result.rows.len() >= 2,
        "should list at least 2 vector indexes"
    );
}
...
fn test_temporal_components() {
    ...
    let result = engine
        .execute(
            &parser
                .parse("RETURN date.month('2024-06-15') AS m")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser
                .parse("RETURN date.quarter('2024-02-15') AS q")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser
                .parse("RETURN date.dayOfWeek('2024-01-01') AS dow")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser
                .parse("RETURN date.dayOfYear('2024-01-15') AS doy")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser
                .parse("RETURN date.truncate('month', '2024-06-15') AS t")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
...
fn test_datetime_components() {
    ...
    let result = engine
        .execute(
            &parser
                .parse("RETURN datetime.year('2024-06-15T14:30:00Z') AS y")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser
                .parse("RETURN datetime.month('2024-06-15T14:30:00Z') AS mo")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser
                .parse("RETURN datetime.day('2024-06-15T14:30:00Z') AS d")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser
                .parse("RETURN datetime.hour('2024-06-15T14:30:00Z') AS h")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser
                .parse("RETURN datetime.minute('2024-06-15T14:30:00Z') AS mi")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser
                .parse("RETURN datetime.second('2024-06-15T14:30:45Z') AS s")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser
                .parse("RETURN datetime.truncate('hour', '2024-06-15T14:30:45Z') AS t")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
...
fn test_time_and_localtime() {
    ...
    assert!(
        !ldt.ends_with('Z'),
        "localdatetime should not end with Z: {ldt}"
    );
}
...
fn test_time_components() {
    ...
    let result = engine
        .execute(
            &parser
                .parse("RETURN time.minute('14:30:45Z') AS mi")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser
                .parse("RETURN time.second('14:30:45Z') AS s")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
...
fn test_call_yield_implicit_return_skip_limit() {
    ...
    engine
        .execute(
            &parser
                .parse("CREATE (n:Person:Dog {name: 'Rover'})")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser
                .parse("CALL db.labels() YIELD label LIMIT 1")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
...
fn test_temporal_param_round_trip() {
    ...
    let result = engine
        ...
        .unwrap();
    assert_eq!(result.rows.len(), 1);
    let created_at = result.rows[0].get("created_at").and_then(Value::as_str);
    assert!(created_at.is_some(), "datetime should survive round-trip");
...
fn test_temporal_param_unwind_nested_row_datetime_round_trips() {
    ...
    assert_eq!(result.rows.len(), 1);
    assert_eq!(
        result.rows[0].get("created_at").and_then(Value::as_str),
        Some(dt)
    );
}
...
fn test_temporal_param_unwind_nested_row_datetime_on_relationship_round_trips() {
    ...
    engine
        .execute(
            &parser
                .parse("CREATE (:X {uuid: 'a'}), (:X {uuid: 'b'})")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser
                .parse(
                    "MATCH (:X)-[r:MENTIONS {uuid: 'm1'}]->(:X) RETURN r.created_at AS created_at",
                )
                .unwrap(),
            &HashMap::new(),
        )
...
fn test_temporal_param_unwind_nested_row_datetime_in_whole_row_maps_round_trips() {
    ...
    assert_eq!(
        rel_result.rows[0].get("created_at").and_then(Value::as_str),
        Some(dt)
    );
}
...
fn test_unwind_relationship_merge_batch_n_arity_match_and_row_replace() {
    ...
    assert_eq!(
        stored.rows[0].get("tenant"),
        Some(&Value::String("tenant-c".to_string()))
    );
    assert_eq!(
...
fn test_unwind_relationship_merge_batch_repeated_match_key_uses_all_row_fields() {
    ...
    let count = engine
        .execute(
            &parser
                .parse(
                    "MATCH (:Component)-[rel:DEPENDS_ON]->(:Component) RETURN count(rel) AS count",
                )
                .unwrap(),
            &HashMap::new(),
        )
...
fn test_chained_call_subquery_implicit_scalar_correlation_optional_aggregate() {
    ...
    assert_eq!(
        result.rows[1].get("person_id"),
        Some(&Value::String("a2".to_string()))
    );
    assert_eq!(
        result.rows[1].get("order_ids"),
...
fn test_relationship_target_inline_filter_special_chars() {
    ...
    let result = engine
        .execute(
            &parser
                .parse(
                    "MATCH (e:Entry)-[:HAS]->(i:Type {name: 'Other Issue'}) RETURN count(e) AS c",
                )
                .unwrap(),
            &HashMap::new(),
        )
...
#[test]
fn grouped_distinct_aggregation_orders_and_limits_aggregated_rows() {
    let engine = make_engine();
    let parser = Parser::new();
    for (id, label, properties) in [
        ("customer:a", "Customer", node_props("A")),
        ("customer:b", "Customer", node_props("B")),
        ("category:alpha", "Category", node_props("Alpha")),
        ("category:beta", "Category", node_props("Beta")),
        ("product:one", "Product", node_props("One")),
        ("product:two", "Product", node_props("Two")),
        ("order:one", "Order", HashMap::new()),
        ("order:two", "Order", HashMap::new()),
        ("order:three", "Order", HashMap::new()),
    ] {
        store_node(engine.storage.as_ref(), id, &[label], properties);
    }
    for (id, start_node, end_node, edge_type) in [
        ("purchased:one", "customer:a", "order:one", "PURCHASED"),
        ("purchased:two", "customer:a", "order:two", "PURCHASED"),
        ("purchased:three", "customer:b", "order:three", "PURCHASED"),
        ("orders:one-alpha", "order:one", "product:one", "ORDERS"),
        (
            "orders:one-alpha-repeat",
            "order:one",
            "product:one",
            "ORDERS",
        ),
        ("orders:one-beta", "order:one", "product:two", "ORDERS"),
        ("orders:two-alpha", "order:two", "product:one", "ORDERS"),
        ("orders:three-beta", "order:three", "product:two", "ORDERS"),
        ("part-of:one", "product:one", "category:alpha", "PART_OF"),
        ("part-of:two", "product:two", "category:beta", "PART_OF"),
    ] {
        engine
            .storage
    ...
    let result = engine
            .execute(
                &parser
                    .parse(
                        "MATCH (c:Customer)-[:PURCHASED]->(o:Order)-[:ORDERS]->(p:Product)-[:PART_OF]->(cat:Category) RETURN c.name AS customer, cat.name AS category, count(DISTINCT o) AS orders ORDER BY orders DESC, customer ASC, category ASC LIMIT 2",
                    )
                    .unwrap(),
                &HashMap::new(),
            )
    ...
    assert_eq!(result.rows[0].get("orders"), Some(&Value::from(2)));
    assert_eq!(
        result.rows[1].get("customer"),
        Some(&Value::String("A".into()))
    );
    assert_eq!(
        result.rows[1].get("category"),
        Some(&Value::String("Beta".into()))
    );
    assert_eq!(result.rows[1].get("orders"), Some(&Value::from(1)));
}

#[test]
fn northwind_optional_match_count_preserves_zero_rows() {
    let engine = make_engine();
    let parser = Parser::new();
    for (id, label, properties) in [
        ("product:a", "Product", node_props("Alpha")),
        ("product:b", "Product", node_props("Beta")),
        ("product:c", "Product", node_props("Charlie")),
        ("order:one", "Order", HashMap::new()),
        ("order:two", "Order", HashMap::new()),
    ] {
        store_node(engine.storage.as_ref(), id, &[label], properties);
    ...
    for (id, start_node, end_node) in [
        ("orders:one", "order:one", "product:a"),
        ("orders:two", "order:two", "product:a"),
        ("orders:three", "order:one", "product:b"),
    ] {
        engine
            .storage
            .put_edge_record(&EdgeRecord {
    ...
    let result = engine
            .execute(
                &parser
                    .parse("MATCH (p:Product) OPTIONAL MATCH (p)<-[r:ORDERS]-(o:Order) RETURN p.name AS productName, count(o) AS orderCount ORDER BY orderCount DESC, productName ASC LIMIT 100")
                    .unwrap(),
                &HashMap::new(),
            )
            .unwrap();
    ...
    assert_eq!(result.rows.len(), 3);
    assert_eq!(
        result.rows[0].get("productName"),
        Some(&Value::from("Alpha"))
    );
    assert_eq!(result.rows[0].get("orderCount"), Some(&Value::from(2)));
    assert_eq!(
        result.rows[1].get("productName"),
        Some(&Value::from("Beta"))
    );
    assert_eq!(result.rows[1].get("orderCount"), Some(&Value::from(1)));
    assert_eq!(
        result.rows[2].get("productName"),
        Some(&Value::from("Charlie"))
    );
    assert_eq!(result.rows[2].get("orderCount"), Some(&Value::from(0)));
}

#[test]
fn northwind_revenue_by_product_aggregates_edge_quantities() {
    let engine = make_engine();
    let parser = Parser::new();
    for (id, label, properties) in [
        (
            ...
            "Product",
            HashMap::from([
                ("productName".into(), Value::from("Alpha")),
                ("unitPrice".into(), Value::from(10.0)),
            ]),
        ),
        (
            "product:b",
            "Product",
            HashMap::from([
                ("productName".into(), Value::from("Beta")),
                ("unitPrice".into(), Value::from(5.0)),
            ]),
        ),
        ("order:one", "Order", HashMap::new()),
        ("order:two", "Order", HashMap::new()),
    ...
    for (id, start_node, end_node, quantity) in [
        ("orders:one", "order:one", "product:a", 2),
        ("orders:two", "order:two", "product:a", 3),
        ("orders:three", "order:one", "product:b", 4),
    ] {
        engine
            .storage
            .put_edge_record(&EdgeRecord {
                ...
                end_node: end_node.into(),
                edge_type: "ORDERS".into(),
                properties: BTreeMap::from([("quantity".into(), Value::from(quantity))]),
                created_at_unix_ms: 0,
                updated_at_unix_ms: 0,
            })
            .unwrap();
    ...
    let result = engine
            .execute(
                &parser
                    .parse("MATCH (p:Product)<-[r:ORDERS]-(:Order) WITH p, sum(p.unitPrice * r.quantity) AS revenue RETURN p.productName AS productName, revenue ORDER BY revenue DESC, productName ASC LIMIT 10")
                    .unwrap(),
                &HashMap::new(),
            )
            .unwrap();
    ...
    assert_eq!(result.rows[0].get("revenue"), Some(&Value::from(50.0)));
    assert_eq!(
        result.rows[1].get("productName"),
        Some(&Value::from("Beta"))
    );
    assert_eq!(result.rows[1].get("revenue"), Some(&Value::from(20.0)));
}
...
fn test_merge_chain_with_match() {
    ...
    let result = engine
            ...
            .unwrap();
    assert_eq!(result.rows.len(), 1);
    assert_eq!(result.rows[0].get("aid"), Some(&Value::String("a1".into())));
...
fn test_merge_with_optional_match() {
    ...
    let result = engine
            ...
            .unwrap();
    assert_eq!(result.rows.len(), 1);
    assert_eq!(result.rows[0].get("aid"), Some(&Value::String("a2".into())));
...
fn test_merge_chain_foreach() {
    ...
    let result = engine
            ...
            .unwrap();
    assert_eq!(result.rows.len(), 1);
    assert_eq!(result.rows[0].get("aid"), Some(&Value::String("a4".into())));
}
...
fn test_aggregation_identity_on_empty_match() {
    ...
    let r = engine
        .execute(
            &parser
                .parse("MATCH (n:Missing) RETURN count(n) AS c")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let r = engine
        .execute(
            &parser
                .parse("MATCH (n:Missing) RETURN avg(n.x) AS a")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let r = engine
        .execute(
            &parser
                .parse("MATCH (n:Missing) RETURN sum(n.x) AS s")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
...
fn test_chained_match_with_aggregation_no_second_match() {
    ...
    engine
        .execute(
            &parser.parse("CREATE (:A {x: 1}), (:A {x: 2})").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
...
fn test_bracket_access() {
    ...
    let result = engine
        .execute(
            &parser
                .parse("WITH $d AS d RETURN d['uuid'] AS uuid")
                .unwrap(),
            &params,
        )
        .unwrap();
...
fn test_set_map_merge_on_matched_node() {
    ...
    engine
        .execute(
            &parser
                .parse("CREATE (n:Test {name: 'Alice', age: 30})")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
...
fn test_merge_on_create_map_merge() {
    ...
    let result = engine
        .execute(
            &parser
                .parse(
                    "MERGE (p:Pds {url: $node.url}) ON CREATE SET p += $node RETURN p.name AS name",
                )
                .unwrap(),
            &params,
        )
    ...
    params.insert(
        "node".to_string(),
        serde_json::json!({"url": "https://example.com", "name": "Updated"}),
    );
    let result = engine
        .execute(
            &parser
                .parse(
                    "MERGE (p:Pds {url: $node.url}) ON MATCH SET p += $node RETURN p.name AS name",
                )
                .unwrap(),
            &params,
        )
...
fn test_merge_multi_label_matching() {
    ...
    engine
        .execute(
            &parser.parse("CREATE (c:FileChunk {id: 'single'})").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let r = engine
        .execute(
            &parser
                .parse("MATCH (n:FileChunk) RETURN n.id AS id ORDER BY id")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
...
fn test_inline_property_match_after_set() {
    ...
    engine
        .execute(
            &parser.parse("CREATE (h:Heuristic {title: 'T'})").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    engine
        .execute(
            &parser
                .parse("MATCH (h:Heuristic {title: 'T'}) SET h.tested_against = 'v'")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
...
fn test_node_key_constraint_enforcement() {
    ...
    let msg = err.to_string();
    assert!(
        msg.contains("already exists") || msg.contains("key"),
        "got: {msg}"
    );
    ...
    let msg = err.to_string();
    assert!(
        msg.contains("cannot be null") || msg.contains("NODE KEY"),
        "got: {msg}"
    );
...
fn test_type_constraint_enforcement() {
    ...
    let msg = err.to_string();
    assert!(
        msg.contains("must be of type") || msg.contains("INTEGER"),
        "got: {msg}"
    );
}
...
fn test_relationship_key_constraint_enforcement() {
    ...
    let msg = err.to_string();
    assert!(
        msg.contains("cannot be null") || msg.contains("RELATIONSHIP KEY"),
        "got: {msg}"
    );
}
...
fn test_relationship_key_unique_enforcement() {
    ...
    let msg = err.to_string();
    assert!(
        msg.contains("already exists") || msg.contains("key") || msg.contains("RELATIONSHIP"),
        "got: {msg}"
    );
}
...
fn test_domain_constraint_enforcement() {
    ...
    let msg = err.to_string();
    assert!(
        msg.contains("DOMAIN") || msg.contains("not in allowed domain"),
        "got: {msg}"
    );
    ...
    engine
        .execute(
            &parser.parse("CREATE (n:Task {name: 'task3'})").unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    engine
        .execute(
            &parser
                .parse("CREATE (n:Task {name: 't4', priority: 1})")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let err = engine
        .execute(
            &parser
                .parse("CREATE (n:Task {name: 't5', priority: 99})")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap_err();
...
fn test_temporal_constraint_enforcement() {
    ...
    engine
        .execute(
            &parser
                .parse(
                    "CREATE (n:FactVersion {fact_key: 'fact1', valid_from: 1000, valid_to: 2000})",
                )
                .unwrap(),
            &HashMap::new(),
        )
    ...
    engine
        .execute(
            &parser
                .parse(
                    "CREATE (n:FactVersion {fact_key: 'fact1', valid_from: 3000, valid_to: 4000})",
                )
                .unwrap(),
            &HashMap::new(),
        )
    ...
    let err = engine
        .execute(
            &parser
                .parse(
                    "CREATE (n:FactVersion {fact_key: 'fact1', valid_from: 1500, valid_to: 2500})",
                )
                .unwrap(),
            &HashMap::new(),
        )
    ...
    let msg = err.to_string();
    assert!(
        msg.contains("TEMPORAL") || msg.contains("overlap"),
        "got: {msg}"
    );
    ...
    let msg = err.to_string();
    assert!(
        msg.contains("TEMPORAL") || msg.contains("cannot be null"),
        "got: {msg}"
    );
}
...
fn test_all_shortest_paths_excludes_longer_paths() {
    ...
    assert_eq!(
        result.rows.len(),
        1,
        "should return only paths at minimum distance"
    );
    for row in &result.rows {
        let hops = row.get("hops").and_then(|v| v.as_i64()).unwrap_or(0);
...
fn test_merge_on_create_set_enforces_constraints_on_updated_properties() {
    ...
    let msg = err.to_string();
    assert!(
        msg.contains("missing") || msg.contains("required") || msg.contains("null"),
        "got: {msg}"
    );
}
...
fn test_merge_on_create_set_enforces_relationship_constraints() {
    ...
    engine
        .execute(
            &parser
                .parse("CREATE (c:Node {name: 'C'}), (d:Node {name: 'D'})")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
...
fn test_vector_index_options_consumed_at_query_time() {
    ...
    let msg = err.to_string();
    assert!(
        msg.contains("dimensions") || msg.contains("expects"),
        "got: {msg}"
    );
}
...
fn test_pattern_comprehension_in_return() {
    ...
    let direct = engine
        .execute(
            &parser
                .parse("MATCH (a:Node {name: 'a'})-[r:LINK]->(m:Node) RETURN m.name AS name")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser
                .parse(
                    "MATCH (a:Node {name: 'a'}) RETURN [(a)-[:LINK]->(m:Node) | m.name] AS names",
                )
                .unwrap(),
            &HashMap::new(),
        )
    ...
    assert_eq!(
        names.len(),
        2,
        "should have 2 reachable nodes, got names: {:?}",
        names
    );
}
...
fn test_call_subquery_importing() {
    ...
    assert_eq!(
        result.rows.len(),
        2,
        "should find 2 reachable nodes via subquery"
    );
}
...
fn test_where_exists_subquery() {
    ...
    let result = engine
        .execute(
            &parser
                .parse(
                    "MATCH (n:Node) WHERE EXISTS { MATCH (n)-[:LINK]->() } RETURN n.name AS name",
                )
                .unwrap(),
            &HashMap::new(),
        )
...
fn test_delete_cleans_up_property_index_entries() {
    ...
    let result = engine
        .execute(
            &parser
                .parse("MATCH (n:Person) RETURN n.name AS name ORDER BY name")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    ...
    let result = engine
        .execute(
            &parser
                .parse("MATCH (n:Person {name: 'Bob'}) RETURN n.name AS name")
                .unwrap(),
            &HashMap::new(),
        )
        .unwrap();
    assert_eq!(
        result.rows.len(),
        1,
        "non-deleted node should still be found"
    );
    assert_eq!(
        result.rows[0].get("name").and_then(|v| v.as_str()),
        Some("Bob")
    ...
    assert_eq!(
        result.rows.len(),
        1,
        "re-created node should be found via index"
    );
}
...
fn test_delete_cleans_up_edge_property_index_entries() {
    ...
    assert_eq!(
        result.rows.len(),
        0,
        "deleted edge should not be found via index"
    );
