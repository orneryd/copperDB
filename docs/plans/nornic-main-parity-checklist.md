...
- [x] Streaming APIs parity (storage now has callback-based node, prefix-scoped node, edge, and node-chunk streaming baselines that walk storage iterators without materializing the whole store; async context cancellation and cache-merge semantics remain tracked under AsyncEngine parity)
...
- [x] Async count/read-path consistency parity (bounded baseline: pending-aware prefix or namespace-label counts plus current label or edge-type reads over the wrapped storage surface)
...
- [x] `crates/storage`: a bounded async-wrapper baseline now exists as `AsyncStorageEngine`, following the same storage-owned seam as NornicDB’s async engine for the current slice: pending structured node/edge writes stay in memory until flush, current-state effective reads merge pending writes over persisted state, pending-aware prefix and namespace-label counts overlay the wrapped storage counters, current label and edge-type reads stay consistent before flush, and the wrapped `StorageEngine` now lives behind a worker-owned message-passing seam so timer-driven background auto-flush does not share sled-backed state across threads. Flush/try-flush return structured results, flush holds block both manual and background flush paths, and MVCC snapshot/lifecycle methods delegate through the worker-owned storage surface. Broader async cache index maintenance remains open.
- [x] `crates/storage`: the async wrapper now also maintains bounded pending label and edge-type inverted indexes instead of rescanning the full pending node/edge maps on every current label or edge-type read. Pending index entries rebuild immediately on label or type changes, evict on delete, and are cleared after successful flush so cached IDs do not accumulate stale scan debt across flush cycles. Broader async cache index maintenance remains open for surfaces the local wrapper does not yet expose.
...
- [ ] `crates/cypher` + `crates/eval`: keep hot-path query optimization parity open for broader UNWIND/MERGE batch routing, call-tail traversal, compound mutation chains, pipeline branch routing, and fuller trace-backed regression coverage. A narrow label-backed `MATCH ... RETURN <var> LIMIT n` route now exists for the simple single-node shape with trace assertions, the current compound create/delete relationship fast path now has deterministic trace coverage, node `MERGE` now records deterministic schema-lookup versus scan-fallback trace behavior, and routed pipeline execution now also covers both the seeded `UNWIND ... MATCH ... CREATE` shape and `UNWIND ... MERGE ... RETURN` for the basic batch upsert shape with trace assertions.
