...
4. In progress: WAL now persists checksummed, versioned transaction frames as one replay unit and can replay complete frames after an applied sequence marker. Storage-engine WAL ownership, applied markers, configured sync modes, replay application, torn-tail handling, corruption diagnostics, and repair remain to do.
