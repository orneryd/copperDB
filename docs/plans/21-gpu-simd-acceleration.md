...
`simd` has real `wide::f32x8` kernels, but zero/error semantics differ from `math`. `gpu` detects libraries/backends while its scoring/k-means implementations still execute CPU loops. Vector search must first become true maintained HNSW under item 10, and Plan 20 must close upstream fixed-stride vector and architecture-specific tolerance changes.
