...
Support status note: copperDB's supported runtime architecture is currently single-node only. References below to distributed packages, partial remote paths, or historical Layer 3 checks are audit findings and parity backlog, not a statement that distributed execution is supported today.
