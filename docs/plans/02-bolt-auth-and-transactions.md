...
- In progress: Phase 5. `RUN` assigns bounded per-session result cursors and returns `qid`; `PULL` and `DISCARD` honor both `n` and `qid`, emit `has_more` only when rows remain, support `qid: -1` for the latest cursor, and reject unknown cursors. Regressions cover partial pull, discard, invalid qid, and multiple cursor selection. Summary counters, notifications, and full bookmark flow remain.
