...
- Complete: declared relationship vector indexes now share the engine-owned registry lifecycle: startup/reopen builds, committed edge create/update/delete callbacks, sidecar persistence, and `db.index.vector.queryRelationships` candidate lookup with ID hydration. Relationship queries no longer scan stored edges; compaction orchestration and Phase 5 tuning/observability remain open.
