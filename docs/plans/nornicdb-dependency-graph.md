...
- `server` now lets HTTP Cypher and Neo4j transaction commit requests opt into distributed Cypher execution with `COPPERDB_DISTRIBUTED_CYPHER` or `x-copperdb-distributed`, using topology-derived placement and quorum consistency while keeping protocol handlers thin. It also exposes authenticated fabric admin routes for shard-map registration/listing, scoped read/search plan inspection, and transport-backed ranked fabric search execution over the internal gRPC ranked-search and hydration RPCs, with list results filtered by the caller's durable per-database access instead of the process default database. The server crate now also builds engine-backed gRPC replica services for local replica apply/read, ranked search, and hydration, and the `copperdb` binary can start that tonic service behind config/CLI gRPC listener settings. The retention admin routes now use the same durable auth gate, allowing read-only principals to inspect retention status/policies while reserving policy, hold, erasure, and sweep mutations for writers.
...
Current Rust status: Layer 4 is in progress. `cypher`, `filter`, and `eval` now share a single parsed expression path for Cypher list and map literals; pattern property maps in `MATCH`/`CREATE`/`MERGE` also evaluate as expressions against the current row, so literal, row-aware, and `UNWIND`-driven shapes execute through the same evaluator path instead of stopping at parser acceptance. The parser and handwritten validator now also retain comma-separated pattern segment boundaries in the AST, which lets `eval` execute disconnected relationship-pattern `MATCH` groups and comma-separated relationship `CREATE` groups without cross-segment misbinding.

`cypher` now matches NornicDB’s query-shape detector more closely for edge-property aggregation, compound-shape probing, and pipeline probing, while `engine` and `eval` consume those routes for real optimized slices: mutual-relationship, incoming/outgoing count aggregation, typed and untyped edge-property aggregation, current compound create/delete relationship fast paths, and parser-approved pipeline chains. That routed pipeline path now threads binding rows itself, reuses bound nodes across chained `CREATE` clauses, constrains later relationship `MATCH` clauses to already-bound endpoints, supports routed `OPTIONAL MATCH` with correct null preservation, supports routed `DELETE`, `SET`, and `REMOVE` after `WITH` without dropping out of the pipeline route, and covers the seeded `UNWIND ... MATCH ... CREATE` seeder shape through both routed execution and direct executor invocation. Eval’s shared mutation paths now distinguish node and relationship bindings for `DELETE`, persist relationship property updates for `SET`, and remove relationship properties plus node labels correctly for `REMOVE`.
